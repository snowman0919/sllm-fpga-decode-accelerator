...
| --- | --- | --- | --- | --- | --- |
| 정직함을 기여처럼 보이면 안 됨 | "하지 않는다" 문장이 초록과 결론에 많았음 | 기여를 Y700 full Gemma probe, Y700 micrograph, ORT profiling, DE10-Lite PoC, 구조 요구사항으로 재정렬 | 초록, 1장, 3장, 8장 | full generation throughput은 아님 | 부분 해결 |
...
| micrograph 사용 당위성이 약함 | 전체 모델 실행 실패의 대체물처럼 읽힐 수 있음 | Y700 full Gemma ONNX CPU EP probe를 추가하고, micrograph는 dense projection dispatch와 compute 비용을 격리하기 위한 실험 설계라고 명시 | 3장, 4장, 8장 | full graph probe는 runs 1 feasibility probe | 해결 |
