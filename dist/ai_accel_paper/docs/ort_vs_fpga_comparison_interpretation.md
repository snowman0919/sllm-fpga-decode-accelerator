...
| --- | --- | --- | ---: | --- |
| CPU NumPy primitive baseline | measured | host wall time around NumPy int32 reference | 29.146000 | host-side primitive baseline; not ONNX Runtime profiling |
| ONNX Runtime MatVec micrograph | measured | ONNX Runtime CPUExecutionProvider session.run wall time | 63.746000 | aligned micrograph baseline only; not full Gemma ONNX Runtime profiling |
...
| FPGA optimized interface estimate | projected | weight-preloaded low-overhead host interface model | 5.135268 | design estimate only; not measured board latency |
