...
DE10-Lite demo top은 선택된 output accumulator의 lower 16-bit를 `HEX3..HEX0`에 표시할 수 있도록 구성하였다. 또한 JTAG-to-Avalon register path는 activation과 weight register를 host에서 기록하고, `start`를 assert한 뒤, result register를 읽어 CPU reference와 비교하는 invocation/correctness path로 구성하였다. 이 design은 full model integration이 아니라 primitive 수준 synthesis, bitstream configuration, board register invocation 증거를 얻기 위한 작은 고정 차원 demo이다.
...
추가로 JTAG-to-Avalon register path에서 fixed INT8 Decode MatVec primitive를 호출한 성공 로그를 반영하였다. 이 로그는 `r0=-271`, `r1=239`, `r2=287`, `r3=797`, `correctness_pass=True`, `interface=jtag_to_avalon`을 기록하며, CPU reference `[-271, 239, 287, 797]`와 일치한다. 단, 제공된 성공 기록에는 numeric JTAG latency breakdown이 archive되어 있지 않으므로 latency field는 비워 둔다. 향후 JTAG latency가 기록되더라도 이는 FPGA 연산부의 순수 compute time이 아니라 System Console 실행, JTAG service 접근, register write/read, polling을 포함한 host-tool invocation overhead로 해석해야 한다. 따라서 본 연구는 JTAG 결과를 성능 가속의 근거로 사용하지 않고, FPGA primitive가 실제 보드 register path에서 호출 가능하며 CPU reference와 동일한 결과를 반환한다는 기능 검증 증거로 사용한다.
...
표 13은 fixed 16x4 primitive 단위에서 CPU reference, ONNX Runtime micrograph, JTAG register invocation을 구분한다. JTAG row는 correctness와 invocation overhead path에 대한 measured evidence이지만, numeric latency가 archive되어 있지 않으므로 latency를 성능 비교 값으로 사용하지 않는다.
...
| --- | --- | --- | --- | ---: | --- | --- |
| CPU NumPy primitive | measured | host NumPy int32 | true | 0.027446 ms | measured CPU reference primitive only | `paper_assets/tables/ort_vs_fpga_projected_comparison.csv` |
| ONNX Runtime MatVec micrograph | measured | CPUExecutionProvider | true | 0.075579 ms | measured ONNX Runtime micrograph baseline only | `paper_assets/tables/ort_vs_fpga_projected_comparison.csv` |
...
표 14는 measured baseline과 optimized FPGA interface estimate를 분리해 정리한다. Compute-only row는 59.85 MHz Quartus slow 85C `CLOCK_50` Fmax와 16 lane, 4 cycle control overhead를 사용한 lower-bound estimate이다. Optimized interface row는 weight preloading, 16 byte activation transfer, 16 byte result transfer, low-overhead driver nominal 50 us를 가정한 projected estimate이다. 이는 measured board latency가 아니며, ONNX Runtime보다 빠르다는 실측 주장으로 해석하지 않는다.

**표 14. ONNX Runtime baseline과 최적화 FPGA interface estimate 비교**

| backend | evidence type | interface | latency | claim boundary | 출처 |
| --- | --- | --- | ---: | --- | --- |
| ONNX Runtime MatVec micrograph | measured | CPUExecutionProvider | 0.075579 ms | measured ONNX Runtime micrograph baseline only | `paper_assets/tables/ort_vs_fpga_projected_comparison.csv` |
| FPGA optimized compute-only | projected | compute-only | 0.000133668 ms | projected estimate, not measured board latency | `paper_assets/tables/fpga_optimized_interface_estimate.csv` |
| FPGA optimized interface | projected | low-overhead driver nominal | 0.050453668 ms | projected estimate, not measured board latency | `paper_assets/tables/fpga_optimized_interface_estimate.csv` |
...
현재 FPGA 결과는 INT8 Decode MatVec primitive가 deterministic vector에 대해 RTL simulation에서 software reference와 일치하고, Quartus compile과 timing summary를 통과하며, DE10-Lite bitstream configuration 및 JTAG-to-Avalon register invocation correctness까지 이어질 수 있음을 보여준다. 이는 전체 sLLM 실행 성능을 증명하는 것이 아니라, profiling-derived projection bottleneck을 hardware primitive로 환원했을 때 FPGA toolchain에서 simulation, synthesis, timing, configuration, register invocation까지 연결 가능함을 보인다.
...
FPGA 측면에서는 INT8 Decode MatVec primitive가 RTL simulation에서 software reference와 일치했고, Quartus resource/timing artifact와 Windows Programmer log를 통해 small fixed-dimension bitstream configuration이 가능함을 확인하였다. 또한 JTAG-to-Avalon register invocation에서 `[-271, 239, 287, 797]` 결과가 CPU reference와 일치함을 기록하였다. 이 증거는 primitive 수준 correctness/invocation 검증으로 제한된다. JTAG latency는 FPGA compute 성능이 아니라 host-tool invocation overhead로 해석하며, optimized FPGA comparison은 measured result가 아니라 projected design estimate로만 제시하였다.
