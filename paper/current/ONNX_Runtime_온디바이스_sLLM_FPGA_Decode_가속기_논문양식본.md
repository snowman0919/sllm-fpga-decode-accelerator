...
추가로 JTAG-to-Avalon register path에서 fixed INT8 Decode MatVec primitive를 20회 반복 호출한 실제 board log를 반영하였다. 이 로그는 `r0=-271`, `r1=239`, `r2=287`, `r3=797`, `correctness_pass=True`, `pass_count=20`, `fail_count=0`, `interface=jtag_to_avalon`을 기록하며, CPU reference `[-271, 239, 287, 797]`와 일치한다. Windows JTAG runner는 run별 `total_wall_latency_ms`, `system_console_elapsed_ms`, `jtag_write_ms`, `jtag_poll_ms`, `jtag_read_ms`, `compute_cycles`, `core_total_cycles`, `debug_status`, service path, cable, Quartus tool path, `.sof` SHA-256을 CSV/JSON/Markdown log로 보존하였다. Primary board-measured evidence는 Windows clean rebuild bitstream SHA-256 `40a4f84167d2fd75972ea33684ec949b6c22e61057f1c134dd5bd4b936ef4a84` 기준이며, 이전 passing bitstream SHA-256 `3b4f2cb50d5aa5608019c550f29b42779ff9c7197383d58cf3132c0bdd635cc5`는 historical prior run으로만 보존한다. 실제 board log에서 `COMPUTE_CYCLES`는 mean/p50/p95 모두 65 cycle이었고, 50 MHz board clock 기준 compute time은 mean/p50/p95 모두 1.3 us이다. 이 값만 FPGA primitive 내부 연산 latency evidence로 사용하며, JTAG total latency는 host-tool invocation overhead로 분리한다.
