...
5. DE10-Lite에서 fixed 16x4 INT8 Decode MatVec primitive를 clean rebuild bitstream으로 구성하고, JTAG-to-Avalon register invocation correctness와 internal cycle counter 기반 65 cycles, 1.3 us @ 50 MHz의 board-measured primitive compute latency를 기록하되, JTAG total latency와 projected interface estimate를 측정 성능 주장과 분리한다.
...
이하 표의 출처 열은 지면 폭을 줄이기 위해 `paper_assets/tables/` 기준 CSV 약어로 표기한다.

| 약어 | CSV |
| --- | --- |
| C1 | `onnx_operator_histogram.csv` |
| C2 | `onnx_graph_io_summary.csv` |
| C3 | `ort_provider_summary.csv` |
| C4 | `ort_context_sweep_latency.csv` |
| C5 | `ort_session_summary.csv` |
| C6 | `ort_long_decode_sweep_latency.csv` |
| C7 | `ort_long_decode_operator_share.csv` |
| C8 | `ort_operator_share_by_context.csv` |
| C9 | `ort_matmul_category_by_context.csv` |
| C10 | `decode_matvec_int8_sim.csv` |
| C11 | `quartus_resource_timing_summary.csv` |
| C12 | `fpga_jtag_primitive_benchmark.csv` |
| C13 | `fpga_jtag_cycle_counter_summary.csv` |
| C14 | `ort_vs_fpga_measured_and_projected_comparison.csv` |
| C15 | `experiment_environment.csv` |
| C16 | `fpga_decode_accel_roofline_estimate.csv` |
| C17 | `onnx_runtime_aligned_micrograph_baseline.csv` |
| C18 | `onnx_runtime_integer_micrograph_baseline.csv` |
| C19 | `fpga_optimized_interface_estimate.csv` |
...
| 항목 | 값 | 출처 |
| --- | ---: | --- |
| ONNX graph node 수 | 7837 | C1 |
| ONNX MatMul node 수 | 237 | C1 |
| cache input 수 | 52 | C2 |
| cache output 수 | 52 | C2 |
| decode cache reuse ready | True | C2 |
...
| 항목 | 설정 또는 산출물 | 출처 |
| --- | --- | --- |
| Runtime provider | ONNX Runtime `CPUExecutionProvider` | C3 |
| Context length | 128, 512, 1024, 2048 | C4 |
| Decode steps | 1, 2, 4, 8 | C4 |
| Runs / warmup | 3 / 1 | C5 |
| Long-decode context length | 128, 512, 2048 | C6 |
| Long-decode steps | 8, 32, 64, 128, 256 | C6 |
| Long-decode runs / warmup | 1 / 0 | C6 |
| Operator share table | operator별 traced node time 비중 | C8 |
| MatMul category table | MatMul node category별 누적 시간 | C9 |
...
| --- | ---: | ---: | ---: | --- |
| prefill + decode trace 수집 node 시간 | 37.918 s | 56.168 s | 67.5% | C8 |
| prefill trace 수집 node 시간 | 14.711 s | 27.539 s | 53.4% | C8 |
| decode trace 수집 node 시간 | 23.207 s | 28.629 s | 81.1% | C8 |
...
그림 2. ONNX Runtime trace 수집 node 시간에서 MatMul이 차지하는 비중. 수치는 C8의 기존 ORT profiling 해석값을 시각화한 것이다.
...
| ---: | ---: | ---: | ---: | ---: | --- |
| 128 | 8 | 139.651 ms | 85.76% | 4.76% | C6, C7 |
| 128 | 256 | 140.496 ms | 86.57% | 4.35% | C6, C7 |
| 512 | 8 | 249.288 ms | 81.32% | 9.20% | C6, C7 |
| 512 | 256 | 203.674 ms | 83.55% | 7.29% | C6, C7 |
| 2048 | 8 | 225.187 ms | 78.33% | 12.01% | C6, C7 |
| 2048 | 256 | 240.789 ms | 72.91% | 17.71% | C6, C7 |
...
| --- | ---: | ---: | ---: | --- |
| `mlp_projection` | 14976 | 23163.290 ms | 61.09% | C9 |
| `lm_head` | 192 | 10546.540 ms | 27.81% | C9 |
| `attention_qkv_projection` | 14976 | 1861.004 ms | 4.91% | C9 |
| `attention_output_projection` | 4992 | 1343.254 ms | 3.54% | C9 |
| `attention_v_weighted_sum` | 4992 | 1001.248 ms | 2.64% | C9 |
| `unknown` | 384 | 2.787 ms | 0.01% | C9 |
| `attention_qk_score` | 0 | 0.000 ms | 0.00% | C9 |
| `mlp_projection + lm_head` | 15168 | 33709.830 ms | 88.90% | C9 |
...
그림 5. C9의 기존 MatMul category 누적 시간을 전체 MatMul 시간 비중으로 시각화한 결과.
...
| ---: | ---: | ---: | --- | ---: | ---: | ---: | --- |
| 0 | -271 | -271 | true | 16 | 4 | 65 | C10 |
| 1 | 239 | 239 | true | 16 | 4 | 65 | C10 |
| 2 | 287 | 287 | true | 16 | 4 | 65 | C10 |
| 3 | 797 | 797 | true | 16 | 4 | 65 | C10 |

표 6은 small fixed-dimension INT8 Decode MatVec primitive가 RTL simulation에서 deterministic software reference와 일치함을 보여준다.

#### 3.6 Quartus clean rebuild 및 DE10-Lite board run 결과

표 7은 JTAG-to-Avalon register wrapper와 internal cycle counter를 포함한 DE10-Lite JTAG Decode MatVec primitive의 clean rebuild resource/timing 결과이다. 이 값은 Windows Pocket4의 Quartus 25.1std에서 `quartus_sh.exe --flow compile de10_lite_jtag_matvec`로 생성한 compile artifact에서 추출하였다.

**표 7. DE10-Lite JTAG Decode MatVec primitive resource and timing summary**

| 항목 | 값 | 출처 |
| --- | ---: | --- |
| Target device | `10M50DAF484C7G` | C11 |
| Total logic elements | 2,560 / 49,760 (5%) | C11 |
| Dedicated logic registers | 1,450 / 49,760 (3%) | C11 |
| Total memory bits | 512 / 1,677,312 (<1%) | C11 |
| Embedded Multiplier 9-bit elements | 1 / 288 (<1%) | C11 |
| Fmax (`CLOCK_50`, worst model) | 56.670 MHz | C11 |
| Worst setup slack / hold slack | 2.353 ns / 0.094 ns | C11 |
| Timing met | True | C11 |
| `.sof` SHA-256 | `40a4f84167d2fd75972ea33684ec949b6c22e61057f1c134dd5bd4b936ef4a84` | C11 |

표 8은 clean rebuild `.sof`를 실제 DE10-Lite에 programming한 뒤 JTAG-to-Avalon register path에서 fixed 16x4 INT8 Decode MatVec primitive를 20회 반복 호출한 board-measured 결과이다. Primary board evidence는 clean rebuild bitstream SHA-256 `40a4f84167d2fd75972ea33684ec949b6c22e61057f1c134dd5bd4b936ef4a84` 기준이며, 이전 passing bitstream SHA-256 `3b4f2cb50d5aa5608019c550f29b42779ff9c7197383d58cf3132c0bdd635cc5`는 historical prior run으로만 보존한다.

**표 8. DE10-Lite JTAG cycle-counter board run summary**

| 항목 | 값 | 출처 |
| --- | ---: | --- |
| pass_count / fail_count | 20 / 0 | C12 |
| reference / result | `-271 239 287 797` / `-271 239 287 797` | C12 |
| `COMPUTE_CYCLES` mean/p50/p95 | 65.0 / 65.0 / 65.0 cycles | C13 |
| compute time mean/p50/p95 | 1.3 / 1.3 / 1.3 us @ 50 MHz | C13 |
| JTAG total latency mean/p50/p95 | 7720.85016 / 7720.45115 / 7748.84833 ms | C12 |
| JTAG latency 해석 | host-tool invocation overhead, not FPGA compute latency | C14 |

JTAG-to-Avalon total latency는 FPGA 연산부의 순수 처리 시간이 아니라 System Console 실행, JTAG service 초기화, register write/read를 포함한 host-tool invocation overhead이다. 따라서 본 연구는 JTAG total latency를 가속 성능의 근거로 사용하지 않고, FPGA 내부 연산부의 처리 시간은 internal cycle counter로 분리해 제시한다. FPGA primitive가 실제 보드에서 호출 가능하며 CPU reference와 동일한 결과를 반환한다는 사실은 기능 검증 증거로 사용하고, 최적화된 interface 비교는 weight preloading 및 DMA/shared-memory style interface를 가정한 projected estimate로 분리해 제시한다.
...
| 항목 | 값 | 출처 |
| --- | --- | --- |
| CPU model | AMD Ryzen 7 9700X 8-Core Processor | C15 |
| CPU logical threads | 16 | C15 |
| RAM | 31939216 kB | C15 |
| OS | Linux-6.17.0-35-generic-x86_64-with-glibc2.42 | C15 |
| Python | 3.13.13 | C15 |
| ONNX Runtime | 1.26.0 | C15 |
| Quartus | Version 25.1std.0 Build 1129 10/21/2025 SC Lite Edition | C15 |
| FPGA device | 10M50DAF484 | C15 |
...
| --- | --- | ---: | ---: | --- | --- |
| `mlp_projection` | 1152 -> 6912 | 9953.28 us | 19978.56 us | bandwidth-bound | C16 |
| `lm_head` | 1152 -> 262144 | 377487.36 us | 757599.04 us | bandwidth-bound | C16 |
| `attention_qkv_projection` | 1152 -> 1024 | 1474.56 us | 2962.24 us | bandwidth-bound | C16 |
| `attention_output_projection` | 1024 -> 1152 | 1474.56 us | 2963.20 us | bandwidth-bound | C16 |
| `attention_v_weighted_sum` | 128 -> 256 | 40.96 us | 84.80 us | bandwidth-bound | C16 |

표 13은 fixed 16x4 primitive 단위에서 CPU reference, ONNX Runtime micrograph, JTAG register invocation, internal cycle-counter row를 구분한다. CPU/ONNX Runtime row는 같은 deterministic activation/weight를 사용하되, CPU row는 INT8 input과 INT32 accumulation이고 ONNX Runtime portable baseline은 float32 MatMul equivalent로 실행하였다. 추가로 같은 INT8 input과 INT32 output boundary를 갖는 ONNX `MatMulInteger` micrograph를 Windows board host 환경에서 별도 baseline으로 실행하였다. 동일한 16x4 INT8 MatVec primitive 조건에서 ONNX Runtime MatMulInteger micrograph는 Windows board host 환경에서 평균 13.012 us를 보였고, FPGA 내부 cycle counter로 측정한 Decode MatVec primitive compute time은 50 MHz 기준 1.3 us였다. 이는 FPGA 내부 연산부가 ONNX Runtime에서 관측된 Decode 병목 primitive를 낮은 compute latency로 처리할 수 있음을 보이는 board-measured evidence이다. 다만 JTAG-to-Avalon total latency는 평균 7720.85 ms로, System Console 실행 및 JTAG service 초기화, register write/read를 포함한 host-tool invocation overhead이므로 end-to-end offload speedup으로 해석하지 않는다.
...
| --- | --- | --- | --- | ---: | --- | --- |
| CPU NumPy primitive | measured | host NumPy int32 | true | 0.028922 ms | measured CPU reference primitive only | C17 |
| ONNX Runtime MatVec micrograph | measured | CPUExecutionProvider | true | 0.063135 ms | measured ONNX Runtime micrograph baseline only; float32 equivalent | C17 |
| ONNX Runtime MatMulInteger micrograph | measured | CPUExecutionProvider | true | 0.013012 ms | measured ONNX Runtime integer micrograph baseline only | C18 |
| FPGA JTAG register offload | measured | JTAG-to-Avalon | true | 7720.85016 ms total invocation mean | measured correctness and invocation overhead, not acceleration | C12 |
| FPGA internal Decode MatVec | measured board cycle counter | cycle counter register | true | 65 cycles, 1.3 us at 50 MHz | primitive internal compute latency only | C13 |
...
표 14는 measured baseline, board-measured FPGA internal compute, JTAG invocation overhead, optimized FPGA interface estimate를 분리해 정리한다. Optimized interface row는 weight preloading, 16 byte activation transfer, 16 byte result transfer, ideal register batch nominal 5 us driver overhead를 가정한 projected estimate이다. 이는 measured board latency가 아니며, ONNX Runtime 전체 실행에 대한 실측 성능 우위 주장으로 해석하지 않는다.

**표 14. ONNX Runtime baseline과 최적화 FPGA interface estimate 비교**

| backend | evidence type | interface | latency | claim boundary | 출처 |
| --- | --- | --- | ---: | --- | --- |
| CPU NumPy primitive | measured | host NumPy int32 | 0.028922 ms | measured host-side primitive baseline | C14 |
| ONNX Runtime MatVec micrograph | measured | CPUExecutionProvider | 0.063135 ms | measured ONNX Runtime micrograph baseline only | C14 |
| ONNX Runtime MatMulInteger micrograph | measured | CPUExecutionProvider | 0.013012 ms | measured integer micrograph baseline only | C14 |
| FPGA internal Decode MatVec | board measured | cycle counter register | 0.001300 ms | primitive internal compute latency only; not end-to-end speedup | C14 |
| FPGA JTAG total invocation | measured | USB-Blaster JTAG / System Console | 7720.850160 ms | invocation overhead; not compute latency | C14 |
| FPGA optimized interface | projected | ideal register batch nominal | 0.005142768 ms | projected estimate, not measured board latency; Fmax from latest Quartus summary | C14 |
...
그림 8. CPU/ONNX Runtime measured baseline, FPGA internal cycle-counter measured primitive latency, JTAG invocation overhead, and FPGA projected interface estimate를 구분한 primitive latency decomposition. JTAG total latency는 host-tool invocation overhead이며 FPGA compute latency로 해석하지 않는다.
...
현재 FPGA 결과는 INT8 Decode MatVec primitive가 deterministic vector에 대해 RTL simulation에서 software reference와 일치하고, Quartus compile과 timing summary를 통과하며, DE10-Lite bitstream configuration 및 JTAG-to-Avalon register invocation correctness까지 이어질 수 있음을 보여준다. 또한 JTAG register bank에는 internal cycle counter가 추가되었고, 실제 20회 board run에서 `COMPUTE_CYCLES=65`, `RESULT=[-271, 239, 287, 797]`, `pass_count=20`, `fail_count=0`을 확인하였다. 이는 전체 sLLM 실행 성능을 증명하는 것이 아니라, profiling-derived projection bottleneck을 hardware primitive로 환원했을 때 FPGA toolchain에서 simulation, synthesis, timing, configuration, register invocation, primitive internal cycle measurement까지 연결 가능함을 보인다.

Future accelerator는 이 primitive를 Host/ORT offload 경계, projection-general weight streaming, output tiling, cache-aware interface와 결합해야 한다. 이때 JTAG-to-Avalon은 low-speed correctness path이므로 future performance interface로 간주하지 않는다. 현재 board-measured 값은 internal cycle counter로 측정한 primitive compute latency에 한정하고, weight preloading 및 DMA/shared-memory 기반 interface 값은 projected estimate로 분리해 제시해야 한다. 따라서 본 절의 구조는 현재 구현 결과를 과장하지 않으면서, ONNX Runtime 병목 분석이 어떤 하드웨어 요구사항으로 이어지는지 명시하는 설계 제안으로 해석해야 한다.
...
FPGA 측면에서는 INT8 Decode MatVec primitive가 RTL simulation에서 software reference와 일치했고, Quartus resource/timing artifact와 Windows Programmer log를 통해 small fixed-dimension bitstream configuration이 가능함을 확인하였다. 또한 JTAG-to-Avalon register invocation에서 `[-271, 239, 287, 797]` 결과가 CPU reference와 일치함을 20회 반복 board run으로 기록하였다. Cycle-counter register와 반복 JTAG logging path를 추가했으며, 실제 board log에서는 `COMPUTE_CYCLES=65`, 50 MHz 기준 compute time 1.3 us, `pass_count=20`, `fail_count=0`을 확인하였다. JTAG latency는 FPGA compute 성능이 아니라 host-tool invocation overhead로 해석하며, optimized FPGA comparison은 measured result가 아니라 projected design estimate로만 제시하였다.
...
여섯째, DE10-Lite board evidence는 clean rebuild `de10_lite_jtag_matvec.sof` bitstream을 실제 board에 programming하고 JTAG-to-Avalon register read로 result vector를 확인한 범위에 한정한다. 현재 JTAG-to-Avalon result vector는 register-read correctness evidence로 반영하였고, 반복 board log는 JTAG total latency와 internal cycle counter를 함께 archive한다. 다만 JTAG total latency는 System Console/JTAG invocation overhead로만 해석하며, FPGA 연산부의 순수 처리 시간으로 사용하지 않는다. FPGA primitive 내부 연산 latency evidence는 real board log에서 읽은 `COMPUTE_CYCLES=65`와 50 MHz 기준 1.3 us 값으로 제한한다.

마지막으로, 성능/대역폭 모델은 design estimate이다. C16과 C19의 compute-bound, bandwidth-bound, 또는 optimized interface latency 값은 architecture exploration을 위한 추정이며, 실제 custom operator 성능 또는 system-level latency reduction을 측정한 결과가 아니다.
