...
| FPGA internal compute cycles | simulation / pending board measurement | cycle counter register | true in RTL simulation | 65 cycles in simulation; board pending | do not report as measured board latency until real board log reads `COMPUTE_CYCLES` | `paper_assets/tables/decode_matvec_regbank_cycle_counter_sim.csv` |
...
현재 FPGA 결과는 INT8 Decode MatVec primitive가 deterministic vector에 대해 RTL simulation에서 software reference와 일치하고, Quartus compile과 timing summary를 통과하며, DE10-Lite bitstream configuration 및 JTAG-to-Avalon register invocation correctness까지 이어질 수 있음을 보여준다. 또한 JTAG register bank에는 internal cycle counter가 추가되었고, register-bank RTL simulation에서 `COMPUTE_CYCLES=65`가 nonzero expected range 안에 있음을 확인하였다. 다만 현재 논문 상태에서 반복 board cycle-counter 측정값은 아직 없으므로 compute cycle latency를 measured board result로 주장하지 않는다. 이는 전체 sLLM 실행 성능을 증명하는 것이 아니라, profiling-derived projection bottleneck을 hardware primitive로 환원했을 때 FPGA toolchain에서 simulation, synthesis, timing, configuration, register invocation까지 연결 가능함을 보인다.
